...
The timetabling literature also considers several side constraints that may be relevant for the quadratic hall-assignment problem and can be accommodated by the three models above. In particular, the ITC 2019 competition model includes a \emph{SameHall} constraint requiring two lectures, scheduled at non-overlapping times, to be assigned to the same hall \citep{muller2025itc2019}. This constraint can be formulated either as a hard constraint that must be satisfied by every feasible solution or as a soft constraint with an associated violation penalty.
Let $C_1^{hard}$ be the set of lecture pairs for which a hard SameHall constraint is imposed. Then the following MIP and MIPQ constraints express it:
\begin{equation}
	\label{eq:hard-SameHall} x_{l_1,h} = x_{l_2,h}     \qquad \forall (l_1,l_2) \in C_1^{hard}, h \in H(l_1) \cap H(l_2),
\end{equation}
and for the CP formulation it is even more straightforward:
\begin{equation}
	\label{eq:hard-SameHall-CP} a_{l_1} = a_{l_2}     \qquad \forall (l_1,l_2) \in C_1^{hard}
\end{equation}
...
Alternatively, suppose that SameHall is a soft constraint for a pair of lectures $l_1,l_2$, with penalty $\pi_{l_1,l_2}$ if the two lectures are assigned to different halls. Let $C_1^{soft}$ be the set of such pairs, and for each pair introduce a binary variable $v_{l_1,l_2}$ that equals $1$ when the constraint is violated. A linear encoding for the MIP formulation is
\begin{equation}
	\label{eq:soft-SameHall-a} v_{l_1,l_2} \ge x_{l_1,h} - x_{l_2,h} \qquad \forall (l_1,l_2) \in C_1^{soft},\ h \in H(l_1) \cap H(l_2),
\end{equation}
\begin{equation}
	\label{eq:soft-SameHall-b} v_{l_1,l_2} \ge x_{l_2,h} - x_{l_1,h} \qquad \forall (l_1,l_2) \in C_1^{soft},\ h \in H(l_1) \cap H(l_2),
\end{equation}
\begin{equation}
	\label{eq:soft-SameHall-c} v_{l_1,l_2} \ge x_{l_1,h} \qquad \forall (l_1,l_2) \in C_1^{soft},\ h \in H(l_1)\setminus H(l_2),
\end{equation}
\begin{equation}
	\label{eq:soft-SameHall-d} v_{l_1,l_2} \ge x_{l_2,h} \qquad \forall (l_1,l_2) \in C_1^{soft},\ h \in H(l_2)\setminus H(l_1).
\end{equation}
...
An important caveat is that SameHall constraints, in both their hard and soft forms, are typically defined over the full weekly timetable. For example, a lecture given on Monday may be required to use the same hall as a related lecture given on Wednesday. If such cross-day constraints are enforced, the problem is no longer decomposable by day, and one must solve at least the whole week, and possibly the whole term, simultaneously.
...
Hard SameHall and SameAttendees constraints can also be incorporated into the compatibility preprocessing step described in Section \ref{subsec:Modeling-improvements}.
